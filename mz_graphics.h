...
mzgr_allocate_cpu_descriptors(mzgr_context_t *gfx,
                              D3D12_DESCRIPTOR_HEAP_TYPE type, u32 count);
...
void mzgr_allocate_upload_memory(mzgr_context_t *gfx, u32 size,
                                 u8 **out_cpu_addr,
                                 D3D12_GPU_VIRTUAL_ADDRESS *out_gpu_addr);

void mzgr_allocate_upload_buffer_region(mzgr_context_t *gfx, u32 size,
                                        u8 **out_cpu_addr,
                                        ID3D12Resource **out_buffer,
                                        u64 *out_buffer_offset);

mzgr_resource_handle_t mzgr_create_committed_resource(
    mzgr_context_t *gfx, D3D12_HEAP_TYPE heap_type, D3D12_HEAP_FLAGS heap_flags,
    const D3D12_RESOURCE_DESC *desc, D3D12_RESOURCE_STATES initial_state,
    const D3D12_CLEAR_VALUE *clear_value);

void mzgr_destroy_resource(mzgr_context_t *gfx, mzgr_resource_handle_t handle);

ID3D12Resource *mzgr_get_resource(mzgr_context_t *gfx,
...
void mzgr_get_back_buffer(mzgr_context_t *gfx,
                          mzgr_resource_handle_t *out_handle,
                          D3D12_CPU_DESCRIPTOR_HANDLE *out_rtv);

void mzgr_get_depth_stencil_buffer(mzgr_context_t *gfx,
                                   mzgr_resource_handle_t *out_handle,
...
_mzgr_allocate_cpu_descriptors(_mzgr_descriptor_heap_t *dh, u32 count) {
...
static _mzgr_descriptor_heap_t
_mzgr_create_descriptor_heap(ID3D12Device *device, u32 capacity,
  ...
  MZGR_VHR(ID3D12Device_CreateDescriptorHeap(
      device, &heap_desc, &IID_ID3D12DescriptorHeap, (void **)&dh.heap));

  ID3D12DescriptorHeap_GetCPUDescriptorHandleForHeapStart(dh.heap,
                                                          &dh.cpu_start);
  if (flags == D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE) {
    ID3D12DescriptorHeap_GetGPUDescriptorHandleForHeapStart(dh.heap,
                                                            &dh.gpu_start);
  }

  dh.descriptor_size =
      ID3D12Device_GetDescriptorHandleIncrementSize(device, type);
  return dh;
}

static _mzgr_gpu_memory_heap_t
_mzgr_create_gpu_memory_heap(ID3D12Device *device, u32 capacity,
  ...
  MZGR_VHR(ID3D12Device_CreateCommittedResource(
      device, &(D3D12_HEAP_PROPERTIES){.Type = type}, D3D12_HEAP_FLAG_NONE,
...
static mzgr_resource_handle_t
_mzgr_add_resource(_mzgr_resource_pool_t *pool, ID3D12Resource *raw,
                   D3D12_RESOURCE_STATES initial_state, DXGI_FORMAT format) {
  assert(pool && raw);
  ...
  assert(slot_idx > 0 && slot_idx <= _MZGR_MAX_NUM_RESOURCES);

  pool->resources[slot_idx].raw = raw;
  pool->resources[slot_idx].state = initial_state;
  pool->resources[slot_idx].format = format;

  return (mzgr_resource_handle_t){(u16)slot_idx,
                                  pool->generations[slot_idx] += 1};
}

static mzgr_pipeline_handle_t _mzgr_add_pipeline(_mzgr_pipeline_pool_t *pool,
                                                 ID3D12PipelineState *pso,
                                                 ID3D12RootSignature *rs) {
  assert(pool && pso && rs);
  ...
  assert(slot_idx > 0 && slot_idx <= _MZGR_MAX_NUM_PIPELINES);

  pool->pipelines[slot_idx].pso = pso;
  pool->pipelines[slot_idx].rs = rs;

  return (mzgr_pipeline_handle_t){(u16)slot_idx,
                                  pool->generations[slot_idx] += 1};
}
...
_mzgr_resource_t *_mzgr_get_resource_state(mzgr_context_t *gfx,
                                           mzgr_resource_handle_t handle) {
  assert(handle.index > 0 && handle.index <= _MZGR_MAX_NUM_RESOURCES);
  assert(handle.generation > 0 &&
         handle.generation == gfx->resource_pool.generations[handle.index]);
...
mzgr_context_t *mzgr_create_context(void *window) {
...
#endif
  if (D3D12CreateDevice(NULL, D3D_FEATURE_LEVEL_11_1, &IID_ID3D12Device,
                        (void **)&gfx->device) < 0) {
    MessageBox(
        window,
  ...
  MZGR_VHR(ID3D12Device_CreateCommandQueue(gfx->device, &cmd_queue_desc,
  ...
  MZGR_VHR(IDXGIFactory4_CreateSwapChain(factory, (IUnknown *)gfx->cmd_queue,
                                         &swap_chain_desc, &temp_swap_chain));
  ...
  for (u32 i = 0; i < _MZGR_NUM_FRAMES; ++i) {
    MZGR_VHR(ID3D12Device_CreateCommandAllocator(
        gfx->device, D3D12_COMMAND_LIST_TYPE_DIRECT,
        &IID_ID3D12CommandAllocator, (void **)&gfx->cmd_alloc[i]));
  }

  gfx->rtv_heap = _mzgr_create_descriptor_heap(gfx->device, 1024,
                                               D3D12_DESCRIPTOR_HEAP_TYPE_RTV,
                                               D3D12_DESCRIPTOR_HEAP_FLAG_NONE);
  gfx->dsv_heap = _mzgr_create_descriptor_heap(gfx->device, 1024,
                                               D3D12_DESCRIPTOR_HEAP_TYPE_DSV,
                                               D3D12_DESCRIPTOR_HEAP_FLAG_NONE);
  gfx->cbv_srv_uav_cpu_heap = _mzgr_create_descriptor_heap(
      gfx->device, 16 * 1024, D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV,
      D3D12_DESCRIPTOR_HEAP_FLAG_NONE);

  for (u32 i = 0; i < _MZGR_NUM_FRAMES; ++i) {
    gfx->cbv_srv_uav_gpu_heaps[i] = _mzgr_create_descriptor_heap(
        gfx->device, 16 * 1024, D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV,
        D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE);
  }

  for (u32 i = 0; i < _MZGR_NUM_FRAMES; ++i) {
    gfx->upload_memory_heaps[i] = _mzgr_create_gpu_memory_heap(
        gfx->device, 8 * 1024 * 1024, D3D12_HEAP_TYPE_UPLOAD);
  }

  gfx->resource_pool.resources =
      MZL_MALLOC((_MZGR_MAX_NUM_RESOURCES + 1) * sizeof(_mzgr_resource_t));
  gfx->resource_pool.generations =
      MZL_MALLOC((_MZGR_MAX_NUM_RESOURCES + 1) * sizeof(u16));

  mzl_memset(gfx->resource_pool.resources, 0,
             (_MZGR_MAX_NUM_RESOURCES + 1) * sizeof(_mzgr_resource_t));
  mzl_memset(gfx->resource_pool.generations, 0,
             (_MZGR_MAX_NUM_RESOURCES + 1) * sizeof(u16));

  gfx->pipeline_pool.pipelines =
      MZL_MALLOC((_MZGR_MAX_NUM_PIPELINES + 1) * sizeof(_mzgr_pipeline_t));
  gfx->pipeline_pool.generations =
      MZL_MALLOC((_MZGR_MAX_NUM_PIPELINES + 1) * sizeof(u16));

  mzl_memset(gfx->pipeline_pool.pipelines, 0,
             (_MZGR_MAX_NUM_PIPELINES + 1) * sizeof(_mzgr_pipeline_t));
  mzl_memset(gfx->pipeline_pool.generations, 0,
             (_MZGR_MAX_NUM_PIPELINES + 1) * sizeof(u16));

  {
    ...
    for (u32 i = 0; i < 4; ++i) {
      ...
      MZGR_VHR(IDXGISwapChain3_GetBuffer(
          gfx->swap_chain, i, &IID_ID3D12Resource, (void **)&buffer));
      gfx->back_buffers[i] = _mzgr_add_resource(
          &gfx->resource_pool, buffer, D3D12_RESOURCE_STATE_PRESENT,
          swap_chain_desc.BufferDesc.Format);
      ID3D12Device_CreateRenderTargetView(
          gfx->device, mzgr_get_resource(gfx, gfx->back_buffers[i]), NULL,
          handle);
...

  {
    ...
    ID3D12Device_CreateDepthStencilView(
        gfx->device, mzgr_get_resource(gfx, gfx->depth_stencil_buffer),
        &view_desc, handle);
  }

  MZGR_VHR(ID3D12Device_CreateCommandList(
      gfx->device, 0, D3D12_COMMAND_LIST_TYPE_DIRECT, gfx->cmd_alloc[0], NULL,
      &IID_ID3D12GraphicsCommandList, (void **)&gfx->cmd_list));
  MZGR_VHR(ID3D12GraphicsCommandList_Close(gfx->cmd_list));

  MZGR_VHR(ID3D12Device_CreateFence(gfx->device, 0, D3D12_FENCE_FLAG_NONE,
                                    &IID_ID3D12Fence,
...
mzgr_resource_handle_t mzgr_create_committed_resource(
  ...
  ID3D12Resource *raw;
  MZGR_VHR(ID3D12Device_CreateCommittedResource(
      gfx->device, &(D3D12_HEAP_PROPERTIES){.Type = heap_type}, heap_flags,
      desc, initial_state, clear_value, &IID_ID3D12Resource, (void **)&raw));

  return _mzgr_add_resource(&gfx->resource_pool, raw, initial_state,
                            desc->Format);
}
...
ID3D12GraphicsCommandList *mzgr_begin_frame(mzgr_context_t *gfx) {
  ...
  ID3D12GraphicsCommandList_SetDescriptorHeaps(
      cmdlist, 1, &gfx->cbv_srv_uav_gpu_heaps[gfx->frame_index].heap);
  return cmdlist;
}

void mzgr_end_frame(mzgr_context_t *gfx, u32 swap_interval) {
  MZGR_VHR(ID3D12GraphicsCommandList_Close(gfx->cmd_list));
  ID3D12CommandQueue_ExecuteCommandLists(gfx->cmd_queue, 1,
                                         (ID3D12CommandList **)&gfx->cmd_list);

  IDXGISwapChain3_Present(gfx->swap_chain, swap_interval, 0);
  ID3D12CommandQueue_Signal(gfx->cmd_queue, gfx->frame_fence,
                            ++gfx->num_frames);

  u64 num_gpu_frames = ID3D12Fence_GetCompletedValue(gfx->frame_fence);

  if ((gfx->num_frames - num_gpu_frames) >= _MZGR_NUM_FRAMES) {
    ID3D12Fence_SetEventOnCompletion(gfx->frame_fence, num_gpu_frames + 1,
  ...
  gfx->back_buffer_index =
      IDXGISwapChain3_GetCurrentBackBufferIndex(gfx->swap_chain);
...
void mzgr_wait_for_gpu(mzgr_context_t *gfx) {
  ID3D12CommandQueue_Signal(gfx->cmd_queue, gfx->frame_fence,
                            ++gfx->num_frames);
  ID3D12Fence_SetEventOnCompletion(gfx->frame_fence, gfx->num_frames,
                                   gfx->frame_fence_event);
  WaitForSingleObject(gfx->frame_fence_event, INFINITE);
...
void mzgr_get_back_buffer(mzgr_context_t *gfx,
  ...
  *out_rtv = gfx->rtv_heap.cpu_start;
  ...
  assert(mzgr_get_resource(gfx, *out_handle));
}
...
void mzgr_allocate_gpu_descriptors(
    mzgr_context_t *gfx, u32 count, D3D12_CPU_DESCRIPTOR_HANDLE *out_cpu_handle,
    D3D12_GPU_DESCRIPTOR_HANDLE *out_gpu_handle) {
...
mzgr_copy_descriptors_to_gpu_heap(mzgr_context_t *gfx, u32 count,
  ...
  ID3D12Device_CopyDescriptorsSimple(gfx->device, count, cpu_base_handle,
                                     src_base_handle,
                                     D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
  return gpu_base_handle;
}

static void _mzgr_allocate_gpu_memory(_mzgr_gpu_memory_heap_t *mheap, u32 size,
                                      u8 **out_cpu_addr,
                                      D3D12_GPU_VIRTUAL_ADDRESS *out_gpu_addr) {
...
void mzgr_allocate_upload_memory(mzgr_context_t *gfx, u32 size,
  ...
  _mzgr_allocate_gpu_memory(&gfx->upload_memory_heaps[gfx->frame_index], size,
                            &cpu_addr, &gpu_addr);

  if (cpu_addr == NULL && gpu_addr == 0) {
    MZGR_VHR(ID3D12GraphicsCommandList_Close(gfx->cmd_list));
    ID3D12CommandQueue_ExecuteCommandLists(
        gfx->cmd_queue, 1, (ID3D12CommandList **)&gfx->cmd_list);
