...
mzgr_resource_handle_t mzgr_create_committed_resource(mzgr_context_t *gfx, D3D12_HEAP_TYPE heap_type,
...
static mzgr_pipeline_handle_t _mzgr_add_pipeline(_mzgr_pipeline_pool_t *pool, ID3D12PipelineState *pso,
                                                 ID3D12RootSignature *rs) {
    assert(pool && pso && rs);
...
mzgr_context_t *mzgr_create_context(void *window) {
    ...
    MZGR_VHR(IDXGIFactory4_CreateSwapChain(factory, (IUnknown *)gfx->cmd_queue, &swap_chain_desc,
                                           &temp_swap_chain));
    MZGR_VHR(
        IDXGISwapChain_QueryInterface(temp_swap_chain, &IID_IDXGISwapChain3, (void **)&gfx->swap_chain));
    ...
    gfx->cbv_srv_uav_cpu_heap = _mzgr_create_descriptor_heap(
        gfx->device, 16 * 1024, D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, D3D12_DESCRIPTOR_HEAP_FLAG_NONE);
    ...
    gfx->resource_pool.resources = MZL_MALLOC((_MZGR_MAX_NUM_RESOURCES + 1) * sizeof(_mzgr_resource_t));
    ...
    gfx->pipeline_pool.pipelines = MZL_MALLOC((_MZGR_MAX_NUM_PIPELINES + 1) * sizeof(_mzgr_pipeline_t));
...

    {
        ...
        for (u32 i = 0; i < 4; ++i) {
            ...
            ID3D12Device_CreateRenderTargetView(
                gfx->device, mzgr_get_resource(gfx, gfx->back_buffers[i]), NULL, handle);
...
void mzgr_allocate_upload_memory(mzgr_context_t *gfx, u32 size, u8 **out_cpu_addr,
    ...
    _mzgr_allocate_gpu_memory(&gfx->upload_memory_heaps[gfx->frame_index], size, &cpu_addr, &gpu_addr);
